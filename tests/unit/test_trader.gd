...
func test_symbolic_places_resolve_from_the_world() -> void:
	...
	assert_almost_eq(depot_pos.x, 26.5, 0.6, "Markt-Depot bei (26, 25) ist dem Spawn-Umfeld am nächsten")
...
func test_trader_preset_walks_full_load_to_the_depot_and_back() -> void:
	...
	player.logout_time = -1e9
	world.advance(20.0)
	assert_eq(player.inventory_count(), data.bali("inventory.capacity"), "voll gesammelt")
	world.advance(60.0)
	var depot := SimTrade.depots(world)[0]
	var stock := SimTrade.depot_stock(depot, "p1")
	assert_gt(int(stock.get("berries", 0)), 0, "Beeren im Depot (abzüglich Gebühr): %s" % [stock])
	...
	for line: String in lines:
		if line.contains("geliefert: Beeren zu nächstes Depot"):
			delivered = true
...
func test_trader_prefers_own_table() -> void:
	...
	assert_eq(int(SimTrade.depot_stock(SimTrade.depots(world)[0], "p1").get("berries", 0)), 0, "nicht zum Depot gelaufen")
