...
func test_market_zone_blocks_damage_building_and_claims() -> void:
	...
	world.apply_damage(victim, 50.0, Vector2.RIGHT, player.id)
	...
	assert_eq(world.can_place(player, "wood_wall", Vector2i(50, 48), 0), "auf dem Markt wird nicht gebaut")
	assert_eq(world.can_place(player, "depot", Vector2i(50, 48), 0), "nicht baubar")
	world.damage_building(depot, 5000.0, player.id)
	assert_true(world.map.buildings.has(depot.id), "unzerstörbar")
	player.pos = Vector2(23.5, 26.5)
	var anchor := world.place_building(player, "anchor", Vector2i(46, 50), 0)  # Kachel (23, 25) neben dem Markt
	assert_not_null(anchor, "Anker: %s" % world.can_place(player, "anchor", Vector2i(46, 50), 0))
...
func test_depot_fee_capacity_and_withdraw() -> void:
	...
	assert_eq(int(world.depot_stock(depot, "p1")["berries"]), 5)
	assert_eq(world.depot_withdraw(player, depot, "stone", 1), "nichts im Depot")
	...
	assert_eq(world.depot_deposit(player, depot, "wood", 20), "")
	assert_eq(world.depot_stock_count(depot, "p1"), capacity, "bis zur Kappe gefüllt")
	assert_eq(int(player.inventory["wood"]), 15, "nur 5 abgegeben (4 behalten + 1 Gebühr)")
	assert_eq(world.depot_deposit(player, depot, "wood", 5), "Depot voll (%d)" % capacity)
	# Fremde haben ihren eigenen Bestand; weit weg geht nichts
	var stranger := world.spawn_player(Vector2(25.5, 24.5), "p2", "Fremder")
	assert_true(world.depot_stock(depot, "p2").is_empty())
	stranger.pos = OPEN
	assert_eq(world.depot_deposit(stranger, depot, "berries", 1), "zu weit weg")
	world.logout(player.id, data.roles["hide"]["rules"])
	assert_eq(world.depot_withdraw(player, depot, "berries", 1), "nur live")
...
func test_npc_delivers_to_depot_and_is_safe_there() -> void:
	...
	assert_eq(int(world.depot_stock(depot, "p1").get("berries", 0)), 8, "Gebühr auch für NPC-Lieferungen")
	var found := false
...
func test_depot_stock_in_save_and_only_own_stock_in_snapshot() -> void:
	...
	player.pos = Vector2(25.5, 25.5)
	player.inventory["berries"] = 10
	world.depot_deposit(player, depot, "berries", 10)
	var stranger := world.spawn_player(Vector2(25.5, 24.5), "p2", "Fremder")
	stranger.inventory["wood"] = 10
	world.depot_deposit(stranger, depot, "wood", 10)
	var copy := SimSave.world_from_dict(data, SimSave.world_to_dict(world))
	assert_eq(int(copy.depot_stock(copy.map.buildings[depot.id], "p1")["berries"]), 8, "Spielstand")
	assert_eq(int(copy.depot_stock(copy.map.buildings[depot.id], "p2")["wood"]), 8)
	world.tick()
	...
	assert_eq(int(mirror.depot_stock(seen, "p2").get("wood", 0)), 8, "eigener Bestand im Spiegel")
	assert_false(mirror.depot_stock(seen, "p2").has("berries"), "fremder Bestand bleibt geheim")
