...
func test_shots_wear_sling_until_it_breaks() -> void:
	...
	assert_eq(world.durability_left(player, "sling"), max_uses, "Start voll")
	var intent := SimIntent.new()
	intent.shoot = true
	world.set_intent(player.id, intent)
	world.tick()
	assert_eq(world.durability_left(player, "sling"), max_uses - 1.0, "ein Schuss = 1")
	world.wear(player, "sling", max_uses - 2.0)
...
func test_melee_and_armor_wear() -> void:
	...
	world.set_intent(player.id, intent)
	world.tick()
	assert_eq(world.durability_left(player, "club"), float(data.items["club"]["durability"]) - 1.0, "Schlag nutzt die Keule ab")
	assert_eq(world.durability_left(victim, "wood_armor"), float(data.items["wood_armor"]["durability"]) - 1.0, "Treffer nutzt die Rüstung des Opfers ab")
	assert_eq(world.durability_left(player, "wood_armor"), float(data.items["wood_armor"]["durability"]), "eigene Rüstung unberührt")
	world.wear(victim, "wood_armor", 100.0)
...
func test_repair_costs_half_and_lowers_maximum_until_unrepairable() -> void:
	player.inventory["wood"] = 20
	assert_eq(world.craft(player, "club"), "")
	assert_eq(world.repair_reason(player, "club"), "nicht abgenutzt")
	world.wear(player, "club", 30.0)
	var cost := world.repair_cost("club")
	assert_eq(int(cost["wood"]), 2, "3 Holz × 0,5 aufgerundet")
	var wood_before := int(player.inventory["wood"])
	assert_eq(world.repair(player, "club"), "")
	...
	assert_almost_eq(world.durability_max(player, "club"), expected_max, 0.001, "Maximum sinkt")
	assert_almost_eq(world.durability_left(player, "club"), expected_max, 0.001, "voll bis zum neuen Maximum")
	var repairs := 1
	while true:
		world.wear(player, "club", 1.0)
		player.inventory["wood"] = 20
		var reason := world.repair(player, "club")
	...
	assert_eq(world.repair_reason(player, "club"), "nur live")


func test_durability_in_save_self_block_and_loot() -> void:
	world.wear(player, "sling", 50.0)
	var copy := SimSave.world_from_dict(data, SimSave.world_to_dict(world))
	assert_eq(copy.durability_left(copy.get_character(player.id), "sling"), world.durability_left(player, "sling"), "Spielstand")
	...
	assert_eq(mirror.durability_left(mirror.get_character(player.id), "sling"), world.durability_left(player, "sling"), "Selbstblock")
	# Beute: Haltbarkeit wandert mit dem Gegenstand
	var victim := world.spawn_player(OPEN + Vector2(0.8, 0), "p2", "Opfer")
	victim.inventory["wood"] = 6
	assert_eq(world.craft(victim, "club"), "")
	world.wear(victim, "club", 10.0)
	world.apply_damage(victim, 1000.0, Vector2.RIGHT, player.id)
	...
	assert_eq(world.durability_left(player, "club"), float(data.items["club"]["durability"]) - 10.0, "abgenutzt wie vorher")
