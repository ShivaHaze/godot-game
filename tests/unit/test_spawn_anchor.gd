...
func test_only_inside_own_claim_and_once() -> void:
	assert_eq(SimConstruction.can_place(world, player, "spawn_anchor", Vector2i(46, 10), 0), "nur im eigenen Claim")
	_found_claim()
	assert_eq(SimConstruction.can_place(world, player, "spawn_anchor", Vector2i(46, 10), 0), "")
	var spawn := SimConstruction.place_building(world, player, "spawn_anchor", Vector2i(46, 10), 0)  # Kachel (23, 5)
	assert_not_null(spawn)
	assert_eq(int(player.inventory["wood"]), 60 - 10 - 2 - 25)
	assert_eq(int(player.inventory["stone"]), 20 - 8)
	player.inventory["wood"] = 60
	assert_eq(SimConstruction.can_place(world, player, "spawn_anchor", Vector2i(44, 12), 0), "du hast schon Spawn-Anker")
	var stranger := world.spawn_player(Vector2(24.5, 6.5), "p2", "Fremder")
	stranger.inventory["wood"] = 30
	stranger.inventory["stone"] = 10
	assert_eq(SimConstruction.can_place(world, stranger, "spawn_anchor", Vector2i(44, 12), 0), "fremder Claim")
	assert_eq(SimConstruction.can_place(world, stranger, "spawn_anchor", Vector2i(50, 12), 0), "nur im eigenen Claim")
...
func test_respawn_next_to_spawn_anchor_else_map_edge() -> void:
	...
	_found_claim()
	var spawn := SimConstruction.place_building(world, player, "spawn_anchor", Vector2i(46, 10), 0)
	assert_not_null(spawn)
	...
	SimConstruction.damage_building(world, spawn, 100.0, -1)
