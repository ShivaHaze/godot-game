...
| `items.json` | Ausrüstung: Waffen (Fern-/Nahkampf, `ammo`, `effect`, `breaks`) und Rüstung (`armor`, `slow`), jeweils mit Werkbank-Kosten und `durability`. |
| `buildings.json` | Bauteile: Größe in Halbzellen, Kosten, Lebenspunkte, Durchlässigkeit (none/owner/all), Verfall je Stunde; Sonderrollen über Flags (`trade`, `sensor_radius`, `trap_damage`, `sign`, `spawn`, `depot`, `next_to_resource`, `claim_only`, `one_per_owner`, `placeable`, `indestructible`). |
