...
- Ausloggen-Menü: 4 Rollen-Presets (Verstecken, Wache, Sammler, Händler), jede Rolle = vorausgefüllte, aufklappbare, editierbare Regelliste. Orte: Hier, Marker, Bauteile und symbolische Orte (eigener Handelstisch, eigener Anker, nächstes Depot), die beim Ausloggen aufgelöst werden.
...
## Spielen
`godot --path .` oder Projekt im Editor öffnen. WASD bewegen, Maus zielen, Linksklick angreifen, E halten sammeln/plündern, F essen, H Verband anlegen (3 s, nicht angreifen), Q Waffe wechseln, C Herstellen (Rüstung anlegen/ablegen, Stationen als Bauteile), B Baumodus (Tab/Mausrad blättert, 1–9 direkt, T drehen, Linksklick setzen, X abreißen/freigeben), E neben dem eigenen Anker liefert Holz ab, E neben dem eigenen Turret lädt Kugeln, E beim Zöllner zahlt den Zoll, E (tippen) neben einem Handelstisch öffnet die Handelstafel, neben einem Markt-Depot die Depot-Tafel, E neben dem eigenen Schild beschriftet es, Enter Chat (/g global, /gi Gilde, /gilde …), M Marker, Esc Ausloggen-Menü, R neuer Charakter nach dem Tod. Im Offline-Modus Knöpfe unten rechts: Zeitsprung, einloggen, gegen sich selbst antreten. Spielstand wird automatisch geführt; 'Neues Spiel' (zweimal klicken) löscht ihn.
