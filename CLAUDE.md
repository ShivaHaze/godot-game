...
server/              Headless-Server: net_server.gd (ENet, Absichten rein, Snapshots raus, Trennen = NPC, Beitritt mit Konto), server_runner.gd
                     (Welt laden, Füllung, Tick, Speichern, Sicherung – für Skript und Programm), accounts.gd (Konten: PBKDF2-Hashes, Ablage
...
```
Server speichert alle 10 s die geänderten Zeilen nach `user://world.db` (SQLite) und legt täglich eine Sicherung in `user://backups/` ab (die letzten 7); Konten liegen in `user://accounts.db`. Ein altes `server_save.dat`/`accounts.dat` wird beim ersten Start übernommen. `runner.data_dir = ""` hält alles im Speicher (Tests). Trennen macht den Charakter zum NPC (Übergang läuft), Wiederkommen unter demselben Namen mit richtigem Passwort loggt in ihn ein; ein Name ist nur einmal gleichzeitig online. Zeitsprung gibt es online nicht, die Zeit läuft für alle.
