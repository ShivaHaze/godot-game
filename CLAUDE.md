...
- Wolf-KI: nähert sich, beißt, flieht bei niedrigem HP, kommt nach; wendet sich gegen Schützen aus der Ferne (jagt den Nächsten im Kreis bis zum Schützen; Leitwolf nur in seinem Revier).
...
- NPC-Modus: gleiches Kampfsystem wie der Spieler, aber schlechter (zielt auf aktuelle statt zukünftige Position, dreht sich zur Bedrohung, handelt vorsichtig: kein Schuss, solange ein Unbeteiligter in der Schusslinie steht – er tritt zur Seite –, Nahkampf nur aufs anvisierte Ziel, Angreifer ist nur, wer in den letzten 5 s traf, ohne ihn nur gewöhnliche Wölfe in Aggro-Reichweite als Ersatzziel, nie der Leitwolf, nie Menschen).
...
**Nicht im Scope:** Netzwerk (kommt als Spike), Bauen, Claims, Gilden, Handel, weitere Rohstoffe, Zustandseffekte, Grafik, Sound, Setting. Keine Assets, keine Addons außer GUT (Tests) und godot-sqlite (Serverpersistenz). Alles sind ColorRects, Linien und Labels.
...
server/              Headless-Server: net_server.gd (ENet, Absichten rein, Snapshots raus, Trennen = NPC, Beitritt mit Konto), server_runner.gd
                     (Welt laden, Füllung nur bei neuer Welt, Neustart-Übergabe live gebliebener Charaktere an ihre Regeln, Tick, Speichern,
                     Sicherung – für Skript und Programm), accounts.gd (Konten: PBKDF2-Hashes, Ablage
                     hinter AccountStore), world_store.gd (Welt in SQLite: Zeilen je Charakter/Bauteil/Claim/Quelle, nur Änderungen,
                     Schema-Versionen, Backup, Import von server_save.dat), account_db_store.gd (Konten in SQLite), server_main.gd (Startskript `-s`)
...
tools/               run_tests.ps1 / run_tests.sh (headless), build.ps1 / build.sh (Exe-Export nach build/), dist/ (Server-starten.bat,
                     ...
                     balance_report.gd (Rollen × Seeds × 8 h headless, druckt Überleben/Vorräte/Hunger/Geschwächte),
                     playtest_sim.gd (Testabend-Nacht headless: Füll-NPCs wie der Server + 8 Test-Charaktere am Spawn, Statistik je Stunde),
                     ...
                     map_gen.gd (Karte als JSON schreiben), build.ps1/build.sh (Export nach build/), deploy.sh (Server per SSH einspielen),
                     dist/ (liegt jedem Release bei: Server-starten.bat, install-server.sh, update-server.sh, LIZENZEN.txt)
addons/gut/          Test-Framework GUT 9.6.1
addons/godot-sqlite/ SQLite als GDExtension (2shady4u, MIT; nur Windows/Linux x86_64) für WorldStore und AccountDbStore
...
Testabend-Nacht: `godot --headless --path . -s tools/playtest_sim.gd -- default 0 16 1` (Karte, Füll-NPCs, Stunden, Seed; optional dt, Berichtsminuten, Datenordner, `guild`/`dump`/`spread`; ≈ 7 min je 16 h bei 20 Hz).
Server-Benchmark: `godot --headless --path . -s tools/server_bench.gd -- 300 5 30` (NPCs, Spieler, Sekunden).

## Netzwerk (Spike)
Exportierte Programme: `Prototyp-Server.exe --headless -- --server 7777 10` (Windows, Konsole über Prototyp-Server.console.exe bzw. Server-starten.bat), `prototyp-server.x86_64 --headless -- --server 7777 10` (Ubuntu; `tools/dist/install-server.sh [Port] [Füllung] [Karte]` richtet den Dienst ein, `update-server.sh` übernimmt alle drei). Füll-NPCs setzt der Server nur in eine neue Welt (Empfehlung 0–20 für 5–10 Spieler). Der Client verbindet über den Startbildschirm (Name, Passwort, host:port). Aus dem Quelltext:
```
godot --headless --path . -s server/server_main.gd -- 7777 10 0      # Server: Port, NPC-Füllung (nur neue Welt), Laufzeit (0 = endlos); Konten mit Passwort
godot --headless --path . -s server/server_main.gd -- 7777 10 0 open # ... offener Server ohne Passwörter (Bots, Rauchtests)
...
```
Server speichert alle 10 s die geänderten Zeilen nach `user://world.db` (SQLite) und legt täglich eine Sicherung in `user://backups/` ab (die letzten 7); Konten liegen in `user://accounts.db`. Ein altes `server_save.dat`/`accounts.dat` wird beim ersten Start übernommen. `runner.data_dir = ""` hält alles im Speicher (Tests). Trennen macht den Charakter zum NPC (Übergang läuft), Wiederkommen unter demselben Namen mit richtigem Passwort loggt in ihn ein; ein Name ist nur einmal gleichzeitig online. Wer beim Stopp des Servers live war, loggt beim nächsten Start mit seinen Regeln aus (Chronik "Server-Neustart"). `world.events` hält die Ereignisse des letzten Ticks plus alles, was seitdem kam (Nachrichten, Tafeln); der nächste `step()` verwirft nur die des vorigen Schritts. Zeitsprung gibt es online nicht, die Zeit läuft für alle.
